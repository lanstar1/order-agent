...
def init_db():
    ...
    # 시드: 기본 프롬프트가 없으면 삽입
    _pt_cnt = cur_or_conn.execute("SELECT COUNT(*) as cnt FROM prompt_templates").fetchone()
    _pt_n = _pt_cnt["cnt"] if isinstance(_pt_cnt, dict) else _pt_cnt[0]
    if _pt_n == 0:
        _prompts = [
            ("story", "system", "시스템 프롬프트", "당신은 20인 유통회사 부사장의 SNS 콘텐츠를 작성합니다.\n주인공: 30대 후반, 경영학과 출신, 코딩 경험 제로 → Claude를 만나 AI 개발에 눈을 뜸.\n반말, 담백한 톤, 관성을 깨는 서사.\n회사명/제품명/직원실명 절대 노출 금지.\n200~400자. 첫 두 줄이 핵심.\n마지막에 해시태그 3~4개."),
            ("story", "inertia_break", "관성 깨기", '소재를 기반으로 "관성 깨기" 글을 쓰세요.\n구조: 관성 장면 → 왜 안 바꿨는지 → AI 적용 → before/after 수치 → 통찰.\n첫 줄: "우리 회사에서 ___는 원래 ___이었다."\n해시태그: #관성깨기\n소재: {source_data}'),
            ("story", "trend_apply", "AI 트렌드 적용기", 'AI 트렌드를 실제 업무에 적용한 결과를 기록하세요.\n구조: 트렌드 소개(한 줄) → 내 업무에 어떻게 적용했는지 → 적용 결과/수치 → 다음에 시도할 것.\n핵심: "해봤다. 결과는 이렇다." 톤. 추측이 아닌 실전 기록.\n해시태그: #AI트렌드적용 #중소기업AI #관성깨기\n소재: {source_data}'),
            ("story", "vp_coding", "부사장 코딩일지", '비개발자 부사장의 코딩 경험 글을 쓰세요.\n구조: 오늘 만든 것 → 삽질 → Claude와 대화 → 자조적 마무리.\n해시태그: #부사장코딩 #비개발자코딩\n소재: {source_data}'),
            ("story", "news_20people", "AI 뉴스 해석", 'AI 뉴스를 20인 회사 관점으로 해석하세요.\n구조: 뉴스 핵심 → 대기업이라면 → 20인 회사라면 → 시사점.\n해시태그: #AI뉴스 #중소기업AI\n뉴스: {source_data}'),
            ("story", "employee_reaction", "직원 반응", '직원 반응 글을 쓰세요. 직원은 역할로만 언급. 저항에 공감.\n구조: 전달 장면+대사 → 첫 반응 → 변화 과정 → 교훈.\n해시태그: #AI도입 #조직변화\n소재: {source_data}'),
            ("story", "weekly_ax", "주간 AX 리포트", '이번 주 AI 전환 회고.\n구조: 핵심 1가지 → 수치 → 발견 → 다음 주 타겟.\n해시태그: #주간AX리포트 #관성깨기\n데이터: {source_data}'),
            ("story", "reels_script", "릴스 스크립트", '릴스 애니메이션 스크립트를 JSON으로 생성하세요.\n\n설정:\n- 주인공: 30대 후반 경영학과 출신 부사장, 코딩 제로 → Claude로 AI 개발\n- 고퀄리티 3D CGI 애니메이션 스타일 (과장된 표정, 큰 눈, 부드러운 피부 질감, 따뜻한 조명)\n- 35~45초, 7개 장면\n- 각 장면: id, start, end, image, motion, subtitle, tts_text, image_prompt, motion_prompt\n- image_prompt에 Pixar, Disney 단어 절대 사용 금지. 대신 stylized 3D CGI, large expressive eyes, smooth rounded features, subsurface scattering skin, cinematic lighting 등 사용\n- 마지막 장면은 반전 또는 유머\n- threads_text: 같은 이야기를 200~400자 텍스트로\n\n소재: {source_data}\n에피소드: {episode_num}\n\nJSON만 출력:'),
            ("image", "character_vp", "VP 캐릭터 마스터", 'A stylized 3D CGI animated character: Korean man in his late 30s, short neat dark hair, black horn-rimmed glasses, slim fit light blue dress shirt with rolled-up sleeves, dark navy chino pants, no tie. Slightly thin build, warm friendly face with large expressive eyes, smooth rounded features, subtle subsurface scattering on skin. Clean minimal office background. Soft cinematic studio lighting with warm ambient fill, high-fidelity 3D render, 4K.'),
            ("image", "character_logistics", "물류 담당자", 'A stylized 3D CGI animated character: Korean man in his 50s, slightly stocky build, short gray-streaked hair, wearing a navy work vest over polo shirt. Arms crossed, skeptical expression. Large expressive eyes, smooth rounded features. Same minimal office background. Soft cinematic lighting, high-fidelity 3D render, 4K.'),
            ("image", "character_accountant", "경리", 'A stylized 3D CGI animated character: Korean woman in her 40s, neat bob haircut, reading glasses on nose tip, holding a calculator, wearing a cream cardigan over white blouse. Precise and organized appearance. Large expressive eyes, smooth rounded features. Same office background. Soft cinematic lighting, high-fidelity 3D render, 4K.'),
            ("image", "character_junior", "영업 막내", 'A stylized 3D CGI animated character: Korean man in his early 20s, slightly messy hair, wireless earbuds, casual hoodie under blazer, energetic expression, holding smartphone. Large expressive eyes, smooth rounded features. Same office background. Soft cinematic lighting, high-fidelity 3D render, 4K.'),
            ("image", "scene_default", "장면 기본 템플릿", '@{character} {action}, stylized 3D CGI, large expressive eyes, smooth rounded features, {lighting}, 9:16 aspect ratio, high-fidelity render, 4K'),
            ("video", "motion_default", "기본 모션", 'subtle breathing motion, slight head movement, natural idle animation, stylized 3D CGI character'),
            ("video", "motion_excited", "발견/흥분 모션", 'character leans forward with excitement, eyes widen, slight smile forms, warm lighting brightens'),
            ("video", "motion_frustrated", "좌절 모션", 'character rubs temples in frustration, then takes a sip of coffee, tired but determined expression'),
            ("video", "motion_skeptical", "의심 모션", 'character crosses arms and tilts head skeptically, slight frown, one eyebrow raised'),
            ("threads", "reels_to_text", "릴스→쓰레드 변환", '릴스 나레이션 스크립트를 쓰레드용 텍스트로 변환하세요.\n원본보다 디테일을 추가하고, 뒷이야기나 수치를 보강하세요.\n200~400자. 해시태그 포함.\n\n릴스 나레이션:\n{narration}\n\n쓰레드 텍스트:'),
        ]
        for _cat, _key, _name, _content in _prompts:
            cur_or_conn.execute(
                "INSERT INTO prompt_templates (category, key, name, content) VALUES (?,?,?,?)",
                (_cat, _key, _name, _content)
            )
