...
async def excluded_export_excel(
    ...
    headers = ["상품주문번호", "상품명", "수령인", "연락처", "주소", "배송메세지", "비고사항", "송장번호"]
    hdr_fill = PatternFill("solid", fgColor="C00000")
    hdr_font = Font(bold=True, color="FFFFFF")
    trk_fill = PatternFill("solid", fgColor="FFFF00")
    for ci, h in enumerate(headers, 1):
        c = ws.cell(1, ci, h)
        c.fill = hdr_fill; c.font = hdr_font; c.alignment = Alignment(horizontal="center")
    ws.cell(1, 8).fill = trk_fill
    ws.cell(1, 8).font = Font(bold=True, color="000000")
    ...
    for o in selected_orders:
        try:
            ...
            po = o.get("productOrder") or {}
            oid = od.get("orderId", "") or po.get("orderId", "")
            if not oid or oid not in excluded_oids:
                continue
            poid = po.get("productOrderId", "")
            if not poid or poid in seen_poid:
                continue
            seen_poid.add(poid)

            prod_name = (po.get("productName", "") or "")
            option    = (po.get("productOption", "") or "")
            if option:
                prod_name = f"{prod_name} [{option}]"
            ...
            ws.cell(row, 1, poid)
            ws.cell(row, 2, prod_name)
            ws.cell(row, 3, rcv)
            ws.cell(row, 4, tel)
            ws.cell(row, 5, full_addr)
            ws.cell(row, 6, cust_msg)
            ws.cell(row, 7, f"{delivery_type} / 전표제외")
            ws.cell(row, 8, "")
            ws.cell(row, 8).fill = PatternFill("solid", fgColor="FFFFCC")
            row += 1
...
@router.post("/kd-dispatch-excel")
async def kd_dispatch_excel(
    file: UploadFile = File(...),
):
    """경동택배 엑셀 업로드 → 네이버 발송처리.
    헤더에서 '상품주문번호'와 '송장번호' 열을 자동 탐색.
    """
    import io
    import openpyxl
    from services.naver_client import naver_client
    ...
    for ci, cell in enumerate(ws[1]):
        ...
        if val in ("상품주문번호", "주문번호", "productOrderId"):
            poid_col = ci
        elif val in ("송장번호", "운송장번호", "trackingNumber"):
            track_col = ci

    if poid_col is None or track_col is None:
        return {"success": False, "error": "엑셀에 '상품주문번호'와 '송장번호' 열이 필요합니다."}

    dispatch_list = []
    skipped = 0
    for row in ws.iter_rows(min_row=2, values_only=False):
        poid = str(row[poid_col].value or "").strip()
        tracking = str(row[track_col].value or "").strip()
        if not poid or not tracking or poid == "None" or tracking == "None":
            skipped += 1
            continue
        dispatch_list.append({
            "productOrderId": poid,
    ...
    if not dispatch_list:
        return {"success": False, "error": f"송장번호가 입력된 행이 없습니다. (빈 행: {skipped}개)"}

    try:
        result = await naver_client.dispatch_orders(dispatch_list)
        result["dispatched_count"] = len(dispatch_list)
        result["skipped_count"] = skipped
        logger.info(f"[SS] 경동발송처리: {len(dispatch_list)}건, 결과={result}")
        return {"success": True, **result}
    except Exception as e:
...
@router.post("/logen-api-dispatch")
async def logen_api_dispatch(
    warehouse: str = Query(..., pattern="^(gimpo|yongsan)$"),
    selected_orders: list[dict] = Body(...),
):
    """
    📮 로젠 OpenAPI 발송 — 기존 "엑셀 다운→로젠사이트→엑셀업로드" 수동 흐름의 API 버전.

    플로우:
      ① orderId 그룹화 + ilogen_orders 빌드 (기존 logen-export-excel과 동일 룰)
      ② 네이버 발주확인 (confirm_orders)
      ③ 로젠 OpenAPI 주문등록 + 송장번호 조회 (register_and_get_slips)
      ④ orderId → slip_no 매핑 후 네이버 발송처리 (dispatch_orders) → '배송중' 전환

    합묶음: qty=1 / mrgYn=N 고정 (기존 엑셀 양식과 동일)
    물품금액(goodsAmt): 그룹 settlementAmount 합계
    """
    from services.naver_client import naver_client
    ...
    if not selected_orders:
        return {"success": True, "message": "선택된 주문이 없습니다.",
                "confirm": None, "logen": None, "dispatch": None, "results": []}
    ...
    for o in selected_orders:
        ...
        if oid not in groups:
            groups[oid] = {"po_first": po, "items": [], "po_ids": []}
        groups[oid]["items"].append(po)
        groups[oid]["po_ids"].append(poid)

    if not groups:
        return {"success": False, "error": "유효한 주문이 없습니다."}

    # ─── ilogen_orders 빌드 (기존 logen-export-excel 규칙 준수) ───
    ilogen_orders = []
    oid_list = []                         # register_and_get_slips 결과 매칭용
    for oid, g in groups.items():
        po_first = g["po_first"]
        addr     = po_first.get("shippingAddress", {}) or {}
        rcv_name = addr.get("name", "") or ""
        tel_home = addr.get("tel1", "") or ""
        tel_cell = addr.get("tel2", "") or tel_home
        rcv_tel  = tel_cell or tel_home
        rcv_addr = ((addr.get("baseAddress", "") or "") + " " +
                    (addr.get("detailedAddress", "") or "")).strip()

        # 운임코드: 착불이면 020, 아니면 030 (신용)
        ship_fee_type = str(po_first.get("shippingFeeType", "") or "")
        fare_ty = "020" if "착불" in ship_fee_type else "030"

        # 배송메모 — 기존 export-excel과 동일 우선순위
        cust_msg = (po_first.get("shippingMemo", "") or
                    po_first.get("deliveryMemo", "") or
                    po_first.get("deliveryMessage", "") or "")
        # 제주 선착불 보정
        if "제주" in rcv_addr and not cust_msg:
            cust_msg = "제주 선착불"

        # 물품명 — 모델명/ERP코드 + 수량 요약 / 물품금액 합계
        model_qty: dict = {}
        goods_amt_total = 0.0
        for item in g["items"]:
            product_id = str(item.get("productId", "") or item.get("productNo", "") or "")
            model = _model_map.get(product_id, "") \
                    or _product_map.get(product_id, "") \
                    or (item.get("productName", "") or "")[:20]
            qty_i = int(item.get("quantity", 1) or 1)
            model_qty[model] = model_qty.get(model, 0) + qty_i
            settle = float(item.get("expectedSettlementAmount", 0) or
                           item.get("totalPaymentAmount", 0) or 0)
            goods_amt_total += settle
        goods_nm = ", ".join(f"{m} x{q}" for m, q in model_qty.items())[:100]

        if not rcv_name or not rcv_addr or not rcv_tel:
            return {"success": False,
                    "error": f"수하인 정보 누락: 주문 {oid} (이름/주소/연락처 확인)"}

        ilogen_orders.append({
            "rcv_name":  rcv_name,
            "rcv_addr":  rcv_addr,
            "rcv_tel":   rcv_tel,
            "qty":       1,                # 합묶음 미사용 — 단일박스 고정
            "goods_nm":  goods_nm,
            "goods_amt": int(round(goods_amt_total)),
            "snd_msg":   cust_msg,
            "fare_ty":   fare_ty,
        })
        oid_list.append(oid)

    logger.info(f"[SS][LogenAPI] 그룹 {len(groups)}개 / 상품주문 {len(all_po_ids)}건 — warehouse={warehouse}")
    ...
    result: dict = {
        ...
        "confirm": None, "logen": None, "dispatch": None,
        "results": [],
    }

    try:
        # ─── ② 네이버 발주확인 ───────────────────────────
        confirm_res = await naver_client.confirm_orders(all_po_ids)
        result["confirm"] = confirm_res
        confirmed_cnt = confirm_res.get("confirmed", 0) if isinstance(confirm_res, dict) else 0
        failed_cnt   = confirm_res.get("failed", 0)   if isinstance(confirm_res, dict) else 0
        # NEW_BEFORE: confirmed>=1 정상.
        # NEW_AFTER : confirmed=0 / failed>0 (이미 처리됨) → 송장 발급 진행.
        # 응답 비정상: confirmed=0 / failed=0 → 진짜 실패, 중단.
        if confirmed_cnt == 0 and failed_cnt == 0:
            err = (confirm_res or {}).get("message") or (confirm_res or {}).get("error") or "발주확인 응답 비정상"
            result["error"] = f"발주확인 실패: {err}"
            return result
        if confirmed_cnt == 0 and failed_cnt > 0:
            fail_errors = confirm_res.get("errors", [])[:3]
            logger.info(f"[SS][LogenAPI] 신규 발주확인 0건 / 이미 처리 {failed_cnt}건 — 송장 발급 진행 (사유 샘플: {fail_errors})")

        # ─── ③ 로젠 OpenAPI 주문등록 + 송장조회 ──────────
        logen_res = await register_and_get_slips(ilogen_orders, warehouse=warehouse)
        result["logen"] = logen_res
        if not logen_res.get("success"):
            result["error"] = logen_res.get("error") or "로젠 송장발급 실패"
            return result

        # results: [{fix_take_no, rcv_name, slip_nos, register_ok}, ...] (입력 순서 유지)
        logen_results = logen_res.get("results", [])

        # ─── ④ dispatch_list 빌드 (orderId 안 모든 productOrderId에 동일 송장) ───
        dispatch_list = []
        ui_results = []
        for idx, oid in enumerate(oid_list):
            if idx >= len(logen_results):
                ui_results.append({"orderId": oid, "rcv_name": "", "slip_no": "", "status": "matching_failed"})
                continue
            r = logen_results[idx]
            slip_nos = r.get("slip_nos", []) or []
            slip_no = slip_nos[0] if slip_nos else ""
            ui_results.append({
                "orderId":  oid,
                "rcv_name": r.get("rcv_name", ""),
                "slip_no":  slip_no,
                "status":   "ok" if slip_no else "no_slip",
            })
            if not slip_no:
                continue
            for poid in groups[oid]["po_ids"]:
                dispatch_list.append({
                    "productOrderId":      poid,
                    "deliveryCompanyCode": "LOGEN",
                    "trackingNumber":      slip_no,
                })

        if not dispatch_list:
            result["dispatch"] = {"dispatched": 0, "message": "발급된 송장이 없어 발송처리 보류"}
            result["results"]  = ui_results
            result["error"]    = "송장번호 미발급"
            return result

        # ─── 네이버 발송처리 (배송중 전환) ──────────────────
        dispatch_res = await naver_client.dispatch_orders(dispatch_list)
        result["dispatch"] = dispatch_res
        result["results"]  = ui_results
        result["success"]  = True
        result["dispatched_count"] = len(dispatch_list)
        return result
        ...
        logger.error(f"[SS][LogenAPI] 처리 오류: {e}", exc_info=True)
        result["error"] = str(e)
        return result
...
async def add_product_map(entry: dict = Body(...)):
    ...
    if not erp_code:
        return {"success": False, "error": "품목코드는 필수입니다."}
    if not prod_no and not model:
        return {"success": False, "error": "상품번호 또는 모델명 중 하나는 필수입니다."}

    # 분기 1: 상품번호 있음 → 상품번호 단위 매핑 (product_map)
    if prod_no:
        ...
        logger.info(f"[SS] 매핑 {action} (product_map): {prod_no} → ERP:{erp_code}, 모델:{model}")
        return {"success": True, "action": action, "target": "product_map",
                "productNo": prod_no, "erpCode": erp_code, "model": model,
                "total": len(_product_map)}

    # 분기 2: 상품번호 없음 + 모델명만 있음 → 추출코드 별칭 (code_alias_map)
    is_new = model not in _code_alias_map
    _code_alias_map[model] = erp_code
    _save_json(SMARTSTORE_CODE_ALIAS_MAP_PATH, _code_alias_map)
    action = "추가" if is_new else "수정"
    logger.info(f"[SS] 매핑 {action} (code_alias_map): {model} → ERP:{erp_code}")
    return {"success": True, "action": action, "target": "code_alias_map",
            "model": model, "erpCode": erp_code,
            "total": len(_code_alias_map)}
